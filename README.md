...
Neste projeto, pratiquei a habilidade de:
...
Este repositório contém um _template_ de uma aplicação **NodeJS** no arquivo _package.json_.
...
É dividida em duas funções: 
  
1 - `isManager` - que será responsável por verificar se uma pessoa colaboradora é gerente ou não. O retorno dessa função deve é um booleano: `true` ou `false`;

2 - `getRelatedEmployees` - que utiliza a primeira função para apresentar as seguintes saídas: 
  * se for uma pessoa colaboradora gerente, retorna um array contendo os nomes das pessoas colaboradoras que ela é responsável;
...
### 9. Função `getOldestFromFirstSpecies`
...
### 10. Função `getEmployeesCoverage`

Esta função será responsável por associar informações de cobertura das pessoas funcionárias.

A cobertura é representada por um objeto com as seguintes propriedades:
...
A função recebe um objeto de opções que determina seu comportamento, sendo:
